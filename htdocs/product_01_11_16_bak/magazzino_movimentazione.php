...
require_once DOL_DOCUMENT_ROOT . '/product/myclass/myMovimentazione.php'; // serve per il ddt
...
} else
{
    ...
    if ($resql)
    {
        ...
        $sel_magazzino = isset($_POST['sel_magazzino']) ? $_POST['sel_magazzino'] : "";
        if (!empty($sel_magazzino))
        {
            if ($sel_magazzino == "OK")
            {
                $id_magazzino = $_REQUEST['magazzino'];
                $id_magazzino;
                $obj_mov = new myMovimentazione($db);
                $array_movimentati = $obj_mov->getMovimentazioniFormMagazzino($id_magazzino);
                ...
                print '</tr>';
                for ($i = 0; $i < count($array_movimentati); $i++)
                {
                    $movimentazione = $array_movimentati[$i];
                    $link_schedaMovimentazione = '<a href="' . DOL_URL_ROOT . '/product/scheda_movimentazione.php?id=' . $movimentazione['codice_mov'] . '">' . $movimentazione['codice_mov'] . '</a></td>';
