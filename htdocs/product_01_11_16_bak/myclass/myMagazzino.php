...
class magazzino
{
    ...
    /**
     * metodo che ritornerà soltanto il magazzino dell'user
     */
    ...
    /** personalizzazione per solari * */
    public function getMagazziniSolari()
    {
        $sql = "SELECT mag.label,mag.rowid ";
        $sql .= " FROM " . MAIN_DB_PREFIX . "entrepot as mag ";
        $sql .= " WHERE label LIKE 'solari' || label LIKE 'st_solari' || label LIKE 'pcm_napoli' || label LIKE 'pcm_milano' || label LIKE 'tpr'";
        $res = $this->db->query($sql);
        if ($res)
        {
            while ($obj_stTec = $res->fetch_array(MYSQLI_ASSOC))
