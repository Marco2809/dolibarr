        ...
        if (is_numeric(GETPOST("nbpiece")) && $id) {
            ...
            if ($product->hasbatch()) {
                ...
                $result = $product->correct_stock_batch(
                        $user, GETPOST("id_entrepot"), GETPOST("nbpiece"), GETPOST("mouvement"), GETPOST("label"), $priceunit, $d_eatby, $d_sellby, GETPOST('batch_number')
                );  // We do not change value of stock for a correction
            } else {
                $result = $product->correct_stock(
                        $user, GETPOST("id_entrepot"), GETPOST("nbpiece"), GETPOST("mouvement"), GETPOST("label"), $priceunit
                );  // We do not change value of stock for a correction
            }
        ...
        if (GETPOST("id_entrepot_source") <> GETPOST("id_entrepot_destination")) {
            if (is_numeric(GETPOST("nbpiece")) && $id) {
                ...
                $result1 = $product->correct_stock(
                        $user, GETPOST("id_entrepot_source"), GETPOST("nbpiece"), 1, GETPOST("label"), $pricesrc
                );

                // Add stock
                $result2 = $product->correct_stock(
                        $user, GETPOST("id_entrepot_destination"), GETPOST("nbpiece"), 0, GETPOST("label"), $pricedest
                );
...
if ($id > 0 || $ref) {
    ...
    if ($result > 0) {
        ...
        if (empty($conf->global->PRODUIT_MULTIPRICES))
            print price($product->price) . ' ' . $langs->trans("HT");
        else
        ...
        $sql = "SELECT f.stock as tot_scorte";
        $sql.= " FROM " . MAIN_DB_PREFIX . "product as f";
        $sql.= " WHERE (f.fk_product_type=2) && (f.ref = '" . $product->ref . "'" . ") ";
        $result = $db->query($sql);
        if ($result) {
            $obj = $db->fetch_object($result);
            $tot_scorte = $obj->tot_scorte;
        }
        $scorta = $product->stock;
        print '<td>' . $tot_scorte;
        ...
        if (!empty($conf->global->STOCK_CALCULATE_ON_SHIPMENT)) {
            ...
            if ($product->stock_theorique != $product->stock_reel)
                ...
                print $langs->trans("RunningOrders");
            print '</td>';
            print '<td>';
        ...
        if ($lastmovementdate) {
            ...
            print '(<a href="' . DOL_URL_ROOT . '/product/stock/mouvement.php?mainmenu=products&idproduct=' . $product->id . '">' . $langs->trans("FullList") . '</a>)';
        } else {
            print '<a href="' . DOL_URL_ROOT . '/product/stock/mouvement.php?mainmenu=products&idproduct=' . $product->id . '">' . $langs->trans("None") . '</a>';
        }
        print "</td></tr>";
    ...
    if ($action == "transfert") {
        ...
        $sql = "SELECT a.cod_famiglia";
        $sql.= " FROM " . MAIN_DB_PREFIX . "asset as a";
        $sql.= " WHERE a.cod_famiglia=" . "'" . $product->ref . "'";
        $res = $db->query($sql);
        $record = $db->fetch_array($res);
        if (empty($record)) {
            print '</td>';
            ...
            print '</td>';
            print '<td width="20%" class="fieldrequired">' . "Codice " . '</td><td width="20%"><input type="text" class="flat" name="nbpiece" size="10" value="' . $record['cod_famiglia'] . '"readonly></td>';
            print '</tr>';
        }
...
if (empty($action) && $product->id) {
    ...
    if ($user->rights->stock->creer) {
        print '<a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?mainmenu=products&id=' . $product->id . '&amp;action=correction">' . $langs->trans("StockCorrection") . '</a>';
    }

    if (($user->rights->stock->mouvement->creer) && !$product->hasbatch()) {
        print '<a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?mainmenu=products&id=' . $product->id . '&amp;action=transfert">' . $langs->trans("StockMovement") . '</a>';
    }
...
if ($resql) {
    ...
    while ($i < $num) {
        ...
        if (empty($conf->global->PRODUIT_MULTI_PRICES))
            print price(price2num($product->price, 'MU'), 1);
        else
            print $langs->trans("Variable");
        print '</td>'; // Ditto : Show PMP from movement or from product
        print '<td align="right">';
        if (empty($conf->global->PRODUIT_MULTI_PRICES))
            print price(price2num($product->price * $obj->reel, 'MT'), 1) . '</td>'; // Ditto : Show PMP from movement or from product
        else
            print $langs->trans("Variable");
        print '</tr>';
        ;
