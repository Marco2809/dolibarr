...
/**
 *	\file       htdocs/core/menus/standard/allscreens_menu.php
 *	\brief      Menu allscreens manager
 */


/**
 *	Class to manage menu allscreens
 */
...
class MenuManager
{
    ...
    function loadMenu($forcemainmenu='',$forceleftmenu='')
    {
        ...
        else
       {
        ...
        $tabMenu=array();
        $menuArbo = new Menubase($this->db,'allscreens');
        $menuArbo->menuLoad($mainmenu, $leftmenu, $this->type_user, 'allscreens', $tabMenu);
    ...
    function showmenu($mode)
    {
        ...
        require_once 'allscreens.lib.php';
        ...
        // left menu 
        if ($mode == 'left') {
            $res=print_left_allscreens_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$this->menu,0);
        }

        // top menu 
        if ($mode == 'top') {
            $res=print_allscreens_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,0);
        }

        if ($mode == 'jmobile')
        {
            print '<!-- JMOBILE MODE -->'."\n";
			// For jmobile menu, we do not invert menus
        	$res=print_allscreens_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$this->menu,1);
        	...
        	foreach($this->menu->liste as $key => $val)		// $val['url','titre','level','enabled'=0|1|2,'target','mainmenu','leftmenu'
        	{
        		...
        		if ($val['enabled'] == 1)
        		{
	        		...
	        		$res=print_left_allscreens_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$submenu,1,$tmpmainmenu,$tmpleftmenu);
