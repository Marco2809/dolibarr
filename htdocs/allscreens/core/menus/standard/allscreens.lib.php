...
/**
 *  \file		htdocs/core/menus/standard/allscreens.lib.php
 *  \brief		Library for file AllScreens menus
 */
require_once DOL_DOCUMENT_ROOT.'/core/class/menubase.class.php';
global $langs;

/**
 * Core function to output top menu allscreens
 *
 ...
 * @return	void
 */
function print_allscreens_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0)
{
...
//    print ' <div class="slidemenu-icon">'."\n";
    print '  <a>'."\n";
...
//    print ' </div>'."\n";
	...
	$showmode=dol_allscreens_showmenu($type_user, $tmpentry, $listofmodulesforexternal);
	if ($showmode)
	{
	...
	if ($showmode)	{
		...
		if (empty($noout)) print_start_menu_entry($idsel,$classname,$showmode);
	    if (empty($noout)) { 
	      if ( $conf->global->DOL_VERSION >='3.7.0' ) {
	        ...
	        print_text_menu_entry($langs->trans("HRM"), $showmode, DOL_URL_ROOT.'/holiday/index.php?mainmenu=hrm&amp;leftmenu=', $id, $idsel, $classname, $atarget);
	      }
	    }
	...
	// Tools
	if ( $conf->global->DOL_VERSION >='3.4.0' && $conf->global->DOL_VERSION <'3.6.0') {
	...
	} else if ( $conf->global->DOL_VERSION >='3.6.0' && $conf->global->DOL_VERSION <'3.7.0') {
	...
	$menuArbo = new Menubase($db,'allscreens');
	$newTabMenu = $menuArbo->menuTopCharger('','',$type_user,'allscreens',$tabMenu);	// Return tabMenu with only top entries

	$num = count($newTabMenu);
	for($i = 0; $i < $num; $i++)	{
		$idsel=(empty($newTabMenu[$i]['mainmenu'])?'none':$newTabMenu[$i]['mainmenu']);

		$showmode=dol_allscreens_showmenu($type_user,$newTabMenu[$i],$listofmodulesforexternal);
		if ($showmode == 1)		{
...
/**
 * Core function to output left menu allscreens
 *
 * @param	DoliDB		$db                 Database handler
 ...
 * @return	void
 */
function print_left_allscreens_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='')
{
	global $user,$conf,$langs,$dolibarr_main_db_name,$mysoc;
	...
	if (empty($noout) && ! empty($conf->global->MAIN_SHOW_LOGO))
	{
		...
		if (! empty($mysoc->logo_mini) && is_readable($conf->mycompany->dir_output.'/logos/thumbs/'.$mysoc->logo_mini))
		{
			...
			print '<div class="company_logo">'."\n";
			print '<center><a href="'.DOL_URL_ROOT.'/index.php?mainmenu=home&amp;leftmenu=" alt="'.$langs->trans(Home).'"><img title="" src="'.$urllogo.'"></a></center>'."\n";
			print '</div>'."\n";
		}
	}
	...
	if ($mainmenu)
	{
				...
				if (! empty($conf->tax->enabled)) {
				  ...
				  if (empty($conf->global->TAX_DISABLE_VAT_MENUS)) {
				    ...
				    if ( $conf->global->DOL_VERSION >= '3.7.0' ) {
				      ...
				      } // end Local Taxes // end Local Taxes 1
			    ...
			    if ( $conf->global->DOL_VERSION >= '3.7.0' ) {
			      ...
			      */
			      ...
			      if (! empty($conf->comptabilite->enabled)) {
			        // Journaux
			        //$newmenu->add("/compta/journaux/index.php?leftmenu=ca",$langs->trans("Journaux"),1,$user->rights->compta->resultat->lire||$user->rights->accounting->comptarapport->lire);
			        //journaux
		...
		*/
		...
		$menuArbo = new Menubase($db,'allscreens');
		$newmenu = $menuArbo->menuLeftCharger($newmenu,$mainmenu,$leftmenu,(empty($user->societe_id)?0:1),'allscreens',$tabMenu);
	...
	if (empty($noout))
	{
		...
		for ($i = 0; $i < $num; $i++)
		{
			...
			$alt++;
			if (empty($menu_array[$i]['level']) && $showmenu)    $blockvmenuopened=true;
			...
			if ($menu_array[$i]['level'] == 0)
			{
				if ($menu_array[$i]['enabled']) {
					print '<li class="menu_titre">'.$tabstring.'<a class="vmenu" href="'.$url.'"'.($menu_array[$i]['target']?' target="'.$menu_array[$i]['target'].'"':'').'>'.(!empty($menu_array[$i]['leftmenu'])?'<div class="icon-'.$menu_array[$i]['leftmenu']." " : '<div class="').'mainvmenu"></div> <span class="mainmenuaspan">'.$menu_array[$i]['titre'].'</span></a>'."\n";
				}
				else if ($showmenu)
				{
					print '<li class="'.$menu_array[$i]['titre'].' menu_titre">'.$tabstring.'<font class="vmenudisabled">'.$menu_array[$i]['titre'].'</font>'."\n";
				}
			}
			...
			if ($menu_array[$i]['level'] > 0)
			{
				if ($menu_array[$i]['enabled'])
				{
				    print '<div class="menu_contenu'.$menu_array[$i]['level'].' mainmenuhspan">'.$tabstring;
 ...
 */
function dol_allscreens_showmenu($type_user, &$menuentry, &$listofmodulesforexternal)
{
	global $conf;
