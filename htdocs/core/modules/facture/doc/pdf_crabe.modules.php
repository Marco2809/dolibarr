...
class pdf_crabe extends ModelePDFFactures
{
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
                ...
                {
					$cig="\r\nNum. Ordine: ".$object->array_options['options_num_ordine']."\r\nCIG: ".$object->array_options['options_cig']."\r\nOggetto: ".$object->array_options['options_oggetto_fatt'];
					$tab_top = 88;

					$pdf->SetFont('','', $default_font_size - 1);
					$pdf->writeHTMLCell(190, 3, $this->posxdesc-1, $tab_top, dol_htmlentitiesbr($cig), 0, 1);
					$nexY = $pdf->GetY();
				...
				{
					$curY = $nexY;
					$pdf->SetFont('','', $default_font_size - 1);   // Into loop to work with multipage || valore 2 su più righe
					$pdf->SetTextColor(0,0,0);
					...
					$pdf->SetFont('','', $default_font_size - 1);   // On repositionne la police par defaut || valore 2 su più righe
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR')
			{
				...
				{
					/*
					$sql = "SELECT fk_bank_account_facturebusinessita";
		            $sql.= " FROM llx_facture_extrafields";
		            $sql.= " WHERE fk_object =".$object->id;
                    $myquery=$this->db->query($sql);
                    $reqValue = $this->db->fetch_array($myquery);
                    $idbanca=$reqValue[0];
                    */
                    
                    $idbanca=$object->array_options['options_fk_bank_account_facturebusinessita'];
					//$bankid=(empty($object->fk_bank)?$conf->global->FACTURE_RIB_NUMBER:$object->fk_bank);
					$bankid=(empty($idbanca)?$conf->global->FACTURE_RIB_NUMBER:$idbanca);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
        ...
        $posz=$this->page_hauteur-$this->marge_basse-20; //posizione ordinate per logo piè di pagina
		...
		// Logo2 a piè di pagina
		/*$logo2=$conf->mycompany->dir_output.'/logos/firma.png';
		if (file_exists($logo2))
		{
			if (is_readable($logo2))
			{
			    $height1=pdf_getHeightForLogo($logo2);
				$pdf->Image($logo2, $this->marge_gauche, $posz, 0, $height1);	// width=0 (auto)
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
				$pdf->SetFont('','B',$default_font_size - 2);
				$pdf->MultiCell(100, 3, $outputlangs->transnoentities("ErrorLogoFileNotFound",$logo2), 0, 'L');
				$pdf->MultiCell(100, 3, $outputlangs->transnoentities("ErrorGoToGlobalSetup"), 0, 'L');
			}
		}
		else
		{
			$text=$this->emetteur->name;
			$pdf->MultiCell(200, 4, $outputlangs->convToOutputCharset($text), 0, 'L');
		}*/
		
$sql = "SELECT esigibilita_iva_facturebusinessita,fk_bank_account_facturebusinessita";
		$sql.= " FROM llx_facture_extrafields";
		$sql.= " WHERE fk_object =".$object->id;
$myquery=$this->db->query($sql);
...
if ($reqValue[0]==3){
$scissione="Scissione dei pagamenti ai sensi art. 17ter del D.P.R. 633/1972";
$pdf->SetFont('','B', $default_font_size - 2);
$pdf->SetXY($this->marge_gauche, $this->marge_basse-88);
$pdf->MultiCell (100,0, $outputlangs->convToOutputCharset($scissione),0,'L');
}
...
//$beneficiario[0]="qualcuno";	
    
	    if ($beneficiario[0]){
			$benef="Beneficiario: ".$beneficiario[0];
$pdf->SetFont('','B', $default_font_size - 2);
$pdf->SetXY($this->marge_gauche, $this->marge_basse-50);
$pdf->MultiCell (100,0, $outputlangs->convToOutputCharset($benef),0,'L');
}	
		...
		if ($showaddress)
		{
			...
			$hautcadre=41; //altezza riquadro dati societari
            ...
            $sql = "SELECT codice_univoco_ufficio_facturebusinessita";
		    $sql.= " FROM llx_societe_extrafields";
		    $sql.= " WHERE fk_object =".$object->client->ref;
            $myquery=$this->db->query($sql);
            $ipa = $this->db->fetch_array($myquery);
            
			// Show recipient information
			$pdf->SetFont('','', $default_font_size - 1);
			$pdf->SetXY($posx+2,$posy+4+(dol_nboflines_bis($carac_client_name,50)*4));
			if (isset($ipa[0])){
			$pdf->MultiCell($widthrecbox, 4, $carac_client."\r\nCodice IPA: ".$ipa[0], 0, 'L');
		    }else{
