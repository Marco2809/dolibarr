...
function supplier_order_pdf_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0)
{
	...
	if ($filefound)
	{
		...
		if ($obj->write_file($object, $outputlangs, $srctemplatepath, $hidedetails, $hidedesc, $hideref) > 0)
		{
            ...
            //uso ghostscript per fare il merge dell'ordine e dei dati contrattuali
            if (strpos($object->ref,'(')===false){
				$object->ref=str_replace("/","_",$object->ref);

			}
            
            $primo = "'".DOL_DATA_ROOT."/fournisseur/commande/".$object->ref."/".$object->ref.".pdf'";
            $secondo = DOL_DATA_ROOT."/fournisseur/coda_ordine.pdf";
            $terzo = "'".DOL_DATA_ROOT."/fournisseur/commande/".$object->ref."/".$object->ref."_1.pdf'";
            
            
            $cmd = "gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=$terzo $primo $secondo";

            $result = shell_exec($cmd);
            $sovrascrittura=shell_exec("mv $terzo $primo");
            // fine ghostscript
