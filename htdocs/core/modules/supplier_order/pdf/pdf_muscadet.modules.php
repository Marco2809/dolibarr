...
class pdf_muscadet extends ModelePDFSuppliersOrders
{
	...
	function write_file($object,$outputlangs='',$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->fournisseur->dir_output.'/commande')
		{
			...
			if (file_exists($dir))
			{
	            ...
	            $heightforfooter = $this->marge_basse + 8;	// 50Height reserved to output the footer (value include bottom margin)
				...
				if (! empty($object->note_private))
				{
					...
					$pdf->SetFont('','', $default_font_size - 3);
					$pdf->writeHTMLCell(190, 3, $this->posxdesc-1, $tab_top, dol_htmlentitiesbr($object->note_private), 0, 1);
					$nexY = $pdf->GetY();
					$height_note=$nexY-$tab_top;
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
					{
						...
						//$pdf->MultiCell($this->posxup-$this->posxtva-1, 3, dol_print_date(serialize($object->line[$i]),'day',false,$outputlangs,true), 0, 'L', 0);
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
	    ...
	    if (!empty($object->array_options["options_tipo_fatturazione"]))
        {
        	$pdf->SetFont('','B', $default_font_size - 2);
        	$pdf->SetXY($this->marge_gauche, $posy);
        	$titre = 'Tipo fatturazione:';
        	$pdf->MultiCell(80, 4, $titre, 0, 'L');

        	$pdf->SetFont('','', $default_font_size - 2);
        	$pdf->SetXY($posxval, $posy);
        	//$lib_mode_reg=$outputlangs->transnoentities("PaymentType".$object->mode_reglement_code)!=('PaymentType'.$object->mode_reglement_code)?$outputlangs->transnoentities("PaymentType".$object->mode_reglement_code):$outputlangs->convToOutputCharset($object->mode_reglement);
        	//$lib_tipo_fatturazione = $object->note_private;
        	$lib_tipo_fatturazione = $object->array_options["options_tipo_fatturazione"];
        	$pdf->MultiCell(70, 4, $lib_tipo_fatturazione,0,'L');
        ...
        if (!empty($object->cond_reglement_code) || $object->cond_reglement)
	    {
			...
			$pdf->MultiCell(80, 6, $lib_condition_paiement,0,'L');
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($object->ref_supplier)
		{
			...
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			//$pdf->MultiCell(100, 3, $outputlangs->transnoentities("RefSupplier")." : " . $outputlangs->convToOutputCharset($object->ref_supplier), '', 'R'); Originale
			$pdf->MultiCell(100, 3, $outputlangs->convToOutputCharset($object->ref_supplier), '', 'R');
		}
		...
		if ($showaddress)
		{
            ...
            $carac_emetteur = str_replace("amministrazione","ufficio.acquisti",$carac_emetteur);
			...
			$pdf->MultiCell(66,5, "Da:", 0, 'L'); //intestazione ordine Da:
			$pdf->SetXY($posx,$posy);
			...
			$pdf->MultiCell($widthrecbox, 5, "A:",0,'L');
