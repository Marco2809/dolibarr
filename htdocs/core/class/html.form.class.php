...
class Form
{
    ...
    function select_thirdparty_list($selected='',$htmlname='socid',$filter='',$showempty=0, $showtype=0, $forcecombo=0, $events=array(), $filterkey='', $outputmode=0, $limit=0)
    {
        ...
        $sql = "SELECT s.rowid, s.nom, s.client, s.fournisseur, s.code_client, s.code_fournisseur, e.codice_univoco_ufficio_facturebusinessita as ipa";
        $sql.= " FROM ".MAIN_DB_PREFIX ."societe as s";
        if (!$user->rights->societe->client->voir && !$user->societe_id) $sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
        $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX."societe_extrafields as e ON s.rowid = e.fk_object"; // ServiceTech
        ...
        if ($resql)
        {
            ...
            if ($num)
            {
                while ($i < $num)
                {
					...
					{
						if ($obj->ipa) $label.=' (IPA: '.$obj->ipa.')'; //ServiceTech
