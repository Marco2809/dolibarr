...
class myPdfIntervento
{
    ...
    public function getCrea()
    {
        ...
        $obj_clienti = new clientiZoccali($this->db);
        $info_magdestinatario = $obj_clienti->getCliente($id_magdestinatario);

        $magazzino_dest_nome = $info_magdestinatario[0]['INSEGNA'];
        ...
        $pdf->MultiCell(0, 0, 'SCHEDA DI INTERVENTO', "LTRB", "C", true, 1);
// ---------------------------------------------------------
...
//$pdf->MultiCell(90, 7, "Documento di trasporto", "LR", 1,false,1,$x,$y);
        ...
        $n = $this->dati['numero_ordine'];
        $data = $this->dati['data_oggi'];
        $doc_trasporto = "N: " . $n . " DEL " . $data;

        $pdf->MultiCell(95, 12, "Scheda di intervento" . "\n" . $doc_trasporto, 1, '', 0, 1, 105, 20, true);
        $pdf->Ln(2);
        $y = $pdf->GetY();

        $destinatario = "DESTINATARIO";

        $destinatario .= "\n" . $info_magdestinatario[0]['INSEGNA'] . "\n" . $info_magdestinatario[0]['INDIRIZZO'] . "\n" . $info_magdestinatario[0]['CITTA'] . " " . $info_magdestinatario[0]['CAP'] . "\n" . "tel. " . $info_magdestinatario[0]['TEL'] . "\n" . "TERMID: " . $info_magdestinatario[0]['TERMID'];

        $pdf->MultiCell(95, 28, $destinatario, "TLBR", 1, false, 1, $pdf->GetX() + 95, $y);

        $pdf->Ln(20);

        $pdf->Image('logo/logo_carta_si.jpg', 90, 0, 9, 9, 'JPG');
...
//$pdf->SetFont('timesB', '',10);
        ...
        for ($i = 0; $i < $n_asset; $i++)
        {
            ...
            $scorta_richiesta = $this->dati['scorta_richieste_pdf'][$asset['cod_asset']];
            $html .= '<td height="15">';
            $html .= " " . $scorta_richiesta;
            $html .= "</td>";
            $html .= "</tr>";
        }
