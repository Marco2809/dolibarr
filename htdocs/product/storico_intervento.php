...
} else
{
    ...
    $statutarray = array();
    $user_id_form_mag = 0;
    for ($j = 0; $j < count($lista_magazzini); $j++)
    {
        $magazzino = $lista_magazzini[$j];
    ...
    for ($j = 0; $j < count($lista_magazzini_dest); $j++)
    {
        $magazzino = $lista_magazzini_dest[$j];
        $elem = $magazzino['label'];
    ...
    if (isset($id_magazzino))
    {
        if (!empty($id_magazzino))
        {
            if ($id_magazzino != -1)
            {
                $array_condizione [] = "l.id_magazzino = " . $id_magazzino;
            }
        }
    }
    ...
    if (isset($codice))
    {
        if (!empty($codice))
        {
            if ($user->tipologia == "T")
            {
                $array_condizione [] = "l.mag_sorgente = " . $id_m . " and l.codice_mov LIKE '%" . $codice . "%'";
            } else
            {
                $array_condizione [] = "l.codice_mov LIKE '" . $codice . "'";
            }
        }
    }
    ...
    if (isset($stato))
    {
        if (!empty($stato))
        {
            if ($stato != -1)
            {
                ...
                if ($flag_stato == 0) // se zero, ovvero non è convalidato
                {// occore poter selezionare anche i non convalidati
                    $flag_convalidazione = " ";
                }
                $array_condizione [] = "l.flag = " . $flag_stato;
            }
        }
    }
    ...
    if (isset($mag_sorgente))
    {
        if (!empty($mag_sorgente))
        {
            if ($mag_sorgente != -1)
            {
                $array_condizione [] = "l.mag_sorgente = " . $mag_sorgente;
            }
        }
    }
    if (isset($mag_dest))
    {
        if (!empty($mag_dest))
        {
            if ($mag_dest != -1)
            {
                if ($user->tipologia == "T")
                {
                    $array_condizione [] = "l.mag_sorgente = " . $id_m . " and l.mag_dest = " . $mag_dest;
                } else
                    $array_condizione [] = "l.mag_dest = " . $mag_dest;
            }
        }
    }
    if (isset($data_convalida))
    {
        ...
        {
            $array_condizione [] = "l.data_convalida LIKE '" . $data_convalida . "'";
        }
    }
        ...
        if (isset($mov_in_out))
        {
            if (!empty($mov_in_out))
            {
                if ($mov_in_out != -1)
                {
                    if ($mov_in_out == 1)
                    {
                        $array_condizione [] = "l.mag_dest = " . $id_m;
                    } else if ($mov_in_out == 2)
                    {
                        $array_condizione [] = "l.mag_sorgente = " . $id_m;
                    }
                } else
                {
                    if (empty($array_condizione))
                        $array_condizione [] = "l.mag_dest = " . $id_m . " OR " . "l.mag_sorgente = " . $id_m;
                }
            } else
            {
                if (empty($array_condizione))
    ...
    //$query .= $where . " ";
    $query = "SELECT * FROM " . MAIN_DB_PREFIX . "form_intervento_zoccali as l " . $and_condition;
    $query .= $where . " ";
    $query.= $db->order("l.data_ritiro", $sortorder);
...
<a id="card" class="tab inline-block" href="' . $root . '/product/nuovo_intervento.php?mainmenu=products&leftmenu=product&type=5" data-role="button">Nuovo intervento</a>
</div>';

        print '<div class="inline-block tabsElem">
<a id="card" class="tabactive tab inline-block" href="' . $root . '/product/storico_intervento.php?mainmenu=products&leftmenu=product&type=5" data-role="button">Storico intervento</a>
</div>';
        ...
        } else
        {
            ...
            print_liste_field_titre("Numero ordine", $_SERVER["PHP_SELF"], "l.flag", $param, "", "", $sortfield, $sortorder);
            print_liste_field_titre("Magazzino di partenza", $_SERVER["PHP_SELF"], "l.mag_sorgente", $param, "", 'align="left"', $sortfield, $sortorder);
            print_liste_field_titre("Cliente", $_SERVER["PHP_SELF"], "l.mag_dest", $param, "", 'align="center"', $sortfield, $sortorder);
            print_liste_field_titre("Doc.Trasporto", $_SERVER["PHP_SELF"], "l.id_ddt", $param, "", 'align="center"', $sortfield, $sortorder);


            // print '<td class="liste_titre" align="right">' . "Data creazione" . '</td>';
            print '<td width="10%">&nbsp;</td>';

            print_liste_field_titre("Data movimentazione", $_SERVER["PHP_SELF"], "l.data_convalida", $param, "", 'align="center"', $sortfield, $sortorder);
            ...
            print '<input class="flat" type="text" name="sref" size="20" value="' . $codice . '">';
            print '</td>';
            ...
            for ($j = 0; $j < count($lista_magazzino); $j++)
            {
                $magazzino = $lista_magazzino[$j];
                $elem = $magazzino['INSEGNA'];
                $statutarray_cliente [$magazzino['TERMID']] = $elem;
            }

            print '<td align="center">';
            print $form->selectarray('mag_dest', $statutarray_cliente, $mag_dest, 1);
            print '</td >';
  ...
  $(function() {
    $( "#datepicker" ).datepicker();
            ...
            while ($i < min($num, $limit))
            {
                ...
                $link_mov = '<a href="' . DOL_URL_ROOT . '/product/scheda_movimentazione.php?mainmenu=products&codice_mov=' . $obj_mov->codice_mov . '" target="_blank">' . $obj_mov->codice_mov . '</a></td>';
                print $link_mov;
                print "</td>\n";
                ...
                $obj_mag_dest = $obj_cliente->getCliente($obj_mov->mag_dest);
                $magazzino_nome_dest = $obj_mag_dest[0]['INSEGNA'];
                print '<td align="center">' . $magazzino_nome_dest . '</td>';


                $link = DOL_URL_ROOT . "/product/ordini/" . $obj_mov->codice_mov . ".pdf";
                print'<td align="center"> <a href="' . $link . '" target="_blank">';
                print'<strong>' . $obj_mov->codice_mov . '</strong>';
                print'</a></td>';
...
function esporta($db, $where_condition)
{
    ...
    $query = "SELECT * FROM " . MAIN_DB_PREFIX . "form_intervento_zoccali as m " . $where_condition;
    $res_query = $db->query($query);
