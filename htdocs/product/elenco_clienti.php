...
} else
{
    ...
    if (isset($codice))
    {
        if (!empty($codice))
        {
            ...
            $array_condizione [] = "a.TERMID LIKE '%" . $codice . "%'";
        }
    }
    ...
    if (isset($etichetta))
    {
        if (!empty($etichetta))
        {
            ...
            $array_condizione [] = "a.INSEGNA LIKE '%" . $etichetta . "%'";
        }
    }
    ...
    $query = "SELECT * ";
    $query .= " FROM " . MAIN_DB_PREFIX . "zoccali_interventi as a " . $where . " ";
    $query.= $db->order("a.TERMID", $sortorder);
    ...
    if ($res_query)
    {
        ...
        print_barre_liste($texte, $page, "elenco_clienti.php", $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, "asset.png");
        ...
        } else
        {
            ...
            // Lignes des titres
            print '<tr class="liste_titre">';
            print_liste_field_titre("TERMID", $_SERVER["PHP_SELF"], "a.TERMID", $param, "", "", $sortfield, $sortorder);
            print_liste_field_titre("Insegna", $_SERVER["PHP_SELF"], "a.INSEGNA", $param, "", "", $sortfield, $sortorder);
            print_liste_field_titre("Citta", $_SERVER["PHP_SELF"], "a.CITTA", $param, "", 'align="left"', $sortfield, $sortorder);

            // print '<td class="liste_titre" align="right">' . "Data creazione" . '</td>';
            print_liste_field_titre("Centro_S", $_SERVER["PHP_SELF"], "a.CENTRO_S", $param, "", 'align="center"', $sortfield, $sortorder);

            print_liste_field_titre("Ragione sociale", $_SERVER["PHP_SELF"], "a.RAG_SOC", $param, "", 'align="center"', $sortfield, $sortorder);
            ...
            print '</td>';
            print '<td class="liste_titre" align="left">';
            print '<input class="flat" type="text" name="snom" size="30" value="' . $etichetta . '">';
            print '</td>';
...
*/
            ...
            while ($i < min($num, $limit))
            {
                ...
                $link_asset = '<a href="' . DOL_URL_ROOT . '/product/scheda_cliente.php?mainmenu=products&TERMID=' . $obj_asset->TERMID . '" >' . $obj_asset->TERMID . '</a></td>';
                print $link_asset;
                print "</td>\n";

                print '<td>' . $obj_asset->INSEGNA . '</td>';
                
                 print '<td>' . $obj_asset->CITTA . '</td>';

           

                print '<td align="center">' . $obj_asset->CENTRO_S . '</td>';

                print '<td align="center">' . $obj_asset->RAG_SOC . '</td>';
                print "</tr>\n";
                $i++;
            }
            //print $excel;
            ...
            $param .=!empty($id_magazzino) ? "&amp;id_magazzino=$id_magazzino" : "";
            print_barre_liste('', $page, "elenco_clienti.php", $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords);
