...
$data1=strtotime($anno."-".$mese."-".$numeroDiGiorni);
...
/*foreach ($vals as $key => $value) {
	// code...
		$sql="SELECT COUNT(cr) as c FROM ost_ticket__cdata as tc INNER JOIN ost_ticket as t ON t.ticket_id = tc.ticket_id WHERE tc.ticket_id = t.ticket_id AND tc.cr ='".trim($key)."' AND t.status_id IN(2,3) AND tc.zz_dt_clmghw >=  ".$data." AND tc.zz_dt_clmghw <=  ".$data1;
		//echo $sql;
...
$sql="SELECT * FROM ost_ticket__cdata as tc INNER JOIN ost_ticket as t ON t.ticket_id = tc.ticket_id AND (tc.cr ='".trim($pt_number)."' OR tc.cr ='".trim($bis)."') AND t.status_id IN(2,3) AND tc.zz_dt_clmghw >=  ".$data." AND tc.zz_dt_clmghw <=  ".$data1;
//echo $sql;
$result = $connessione->query($sql);
...
if($_POST['commessa']==1)
{
	$sql_fact1="SELECT tc.ref_num,hp.topic FROM ost_ticket__cdata as tc INNER JOIN ost_ticket as t ON t.ticket_id =  tc.ticket_id INNER JOIN ost_help_topic as hp ON t.topic_id=hp.topic_id WHERE REPLACE(tc.cr,'bis','') NOT IN($string) AND t.status_id IN(2,3) AND tc.zz_dt_clmghw >=  ".$data." AND tc.zz_dt_clmghw <=  ".$data1." AND t.topic_id NOT IN (12,13,14,15,16,17,39,40,41,42,43)" ;
	$result_fact1=$connessione->query($sql_fact1);
	echo "<br><b>TICKET NON PRESENTI NEL FILE COOPERSYSTEM:</b><br>";
} else if($_POST['commessa']==2)
{
	$sql_fact1="SELECT tc.ref_num,hp.topic FROM ost_ticket__cdata as tc INNER JOIN ost_ticket as t ON t.ticket_id =  tc.ticket_id INNER JOIN ost_help_topic as hp ON t.topic_id=hp.topic_id WHERE REPLACE(tc.cr,'bis','') NOT IN($string) AND t.status_id IN(2,9) AND tc.zz_dt_clmghw >=  ".$data." AND tc.zz_dt_clmghw <=  ".$data1." AND t.topic_id IN (12,13,14,15,16,17,39,43)" ;
	$result_fact1=$connessione->query($sql_fact1);
	echo "<br><b>TICKET NON PRESENTI NEL FILE CARTASI:</b><br>";
} else if($_POST['commessa']==3)
{
	$sql_fact1="SELECT tc.ref_num,hp.topic FROM ost_ticket__cdata as tc INNER JOIN ost_ticket as t ON t.ticket_id =  tc.ticket_id INNER JOIN ost_help_topic as hp ON t.topic_id=hp.topic_id WHERE REPLACE(tc.cr,'bis','') NOT IN($string) AND t.status_id IN(2,9) AND tc.zz_dt_clmghw >=  ".$data." AND tc.zz_dt_clmghw <=  ".$data1." AND t.topic_id IN (40,41,42)" ;
	$result_fact1=$connessione->query($sql_fact1);
