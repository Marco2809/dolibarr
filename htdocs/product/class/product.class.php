...
// Ne pas mettre de quote sur les numeriques decimaux.
        ...
        if (isset($this->sousprods) && is_array($this->sousprods))
        {
            foreach ($this->sousprods as $prod_name => $desc_product)
            {
                if (is_array($desc_product))
                    $this->fetch_prod_arbo($desc_product, "", $multiply);
            }
        }
        //var_dump($this->res);
        return $this->res;
    ...
    function getNomUrl($withpicto = 0, $option = '', $maxlength = 0)
    {
        ...
        {
            if ($this->type == 0) //aggiunto da amin
            {
                $lien = '<a href="' . DOL_URL_ROOT . '/product/fiche.php?mainmenu=products&id=' . $this->id . '" target="_blank">';
            } else if ($this->type == 2)
            {
                $lien = '<a href="' . DOL_URL_ROOT . '/product/scheda_famiglia.php?mainmenu=products&id=' . $this->ref . '" target="_blank">';
            }
            $lienfin = '</a>';
        }
        $newref = $this->ref;
        ...
        if ($withpicto)
        {
            ...
            if ($this->type == 2)
            {
                $path_img_famiglia = DOL_URL_ROOT."/theme/eldy/img/famiglia.png";
                $oggetto_view = img_object("Visualizza famiglia". ' ' . $this->ref, 'famiglia');
                $result.=($lien . $oggetto_view . $lienfin . ' ');
            }
