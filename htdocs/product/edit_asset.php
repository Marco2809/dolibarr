...
} else
{
    ...
    if ($resql)
    {
        ...
        if ($salva_button == "salva")
        {
            ...
            $flag_qt_uno = true;
            if ($scorta_totale < $scorta_delete) // se la quantità da eliminare è maggiore  della quantità totale
            {
                $str_msg = '<div> Quantita pezzi HW da rimuovere è <b> inferiore alla quantita totale</b></div>';
                msgErrore($str_msg);
                $flag_qt_uno = false;
