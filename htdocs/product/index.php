...
if (! empty($conf->product->enabled)) // Statistiche menu
{
...
if ($result)
{
	...
	if ($num > 0) // ultimi 15 prodotti
	{
		...
		if (isset($_GET["type"]) && $_GET["type"] == 1) $transRecordedType = $langs->trans("LastRecordedServices",$max);
                if (isset($_GET["type"]) && $_GET["type"] == 2) $transRecordedType = $langs->trans("LastRecordedFamily",$max);
		...
		while ($i < $num)
		{
			...
			$var=!$var;
			print "<tr ".$bc[$var].">";
			print '<td class="nowrap" style="width:25%;">';
			...
			print '<td align="right" class="nowrap" style="width:15%;">';
