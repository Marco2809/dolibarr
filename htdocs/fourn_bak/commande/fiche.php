...
else if ($action == 'addline' && $user->rights->fournisseur->commande->creer)
{
    ...
    else if((GETPOST('price_ht')!=='' || GETPOST('price_ttc')!=='') && empty($error))
	{
    	...
    	if (!empty($_POST['price_ht']))
    	{
    		...
    		$result=$object->addline($desc, $ht, $qty, $tva_tx, $localtax1_tx, $localtax2_tx, 0, 0, '', $remise_percent, $price_base_type, 0, $type,0,0,$date_start,$date_end);//Service Tech
    	}
    	else
    	{
    		...
    		$price_base_type = 'HT';
    		$result=$object->addline($desc, $ht, $qty, $tva_tx, $localtax1_tx, $localtax2_tx, 0, 0, '', $remise_percent, $price_base_type, $ttc, $type,0,0,$date_start,$date_end); //Service Tech
    	}
	}
...
else if ($action == 'add' && $user->rights->fournisseur->commande->creer)
{
    ...
    if (! $error)
    {
        ...
        // Creation commande
        //$object->ref_supplier  	= GETPOST('refsupplier')."\r\nData ordine: ".trim($_POST['options_data_ordine']); //aggiungo la data ordine
        $object->ref_supplier = "Data emissione: ".trim($_POST['options_data_ordine']); //aggiungo la data ordine
        $object->socid = $socid;
	    $object->cond_reglement_id = GETPOST('cond_reglement_id');
        $object->mode_reglement_id = GETPOST('mode_reglement_id');
        //$object->note_private	= GETPOST('note_private');
        //$object->note_private = trim($_POST['options_tipo_fatturazione']);
        $cerca = array("\\",  "\x00", "\n",  "\r",  "'",  '"', "\x1a");
        $rimpiazza = array("\\\\","\\0","\\n", "\\r", "\'", '\"', "\\Z");
        $object->note_private	=  str_replace( $cerca, $rimpiazza, GETPOST('note_private'));
        $object->note_public   	= "Rif. Offerta: ".trim($_POST['options_rif_offerta'])." - Rif. Contatto: ".trim($_POST['options_rif_contatto'])."\r\n\r\nNote:\r\n".str_replace( $cerca, $rimpiazza, GETPOST('note_public'));
...
if ($action=="create")
{
	...
	print '<td>';
	//$doleditor = new DolEditor('note_public', GETPOST('note_public'), 'xxx', 80, 'dolibarr_notes', 'In', 0, false, true, ROWS_3, 70);
	//print $doleditor->Create(1);
        print '<textarea name="note_public" wrap="soft" cols="80" rows="'.ROWS_5.'" >Al presente ordine si applica l\'allegato "Contratto di Servizi". Entrambi i documenti devono essere firmati e trasmessi alla mail ufficio.acquisti@service-tech.org. Nel caso in cui il suddetto ordine non venga inviato a mezzo mail entro 7 giorni dal suo ricevimento, si intenderà accettato in ogni sua parte. I prezzi sono da intendersi iva esclusa. In fattura indicare sempre il nostro numero d\'ordine.</textarea>';
	print '</td>';
	...
	//print $doleditor->Create(1);
	//print '</td>';
	print '<td><textarea name="note_private" wrap="soft" cols="60" rows="'.ROWS_5.'">Il Richiedente dichiara di conoscere ed accettare espressamente, ai sensi e per gli effetti degli artt. 1341-1342 cod. civ., le seguenti clausole delle Condizioni Generali Contrattuali (allegato A):

Art. 3 – Condizioni Generali di Fornitura
Art. 4 – Responsabilità
Art. 6 – Certificazione del Fornitore
Art. 7 – D.lgs 196/2003 “Codice in materia di protezioni dati personali”
Art. 8 – Riservatezza
