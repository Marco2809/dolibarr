    ...
    if ($resql)
    {
        ...
        if ($salva_button == "salva")
        {
            ...
            if ($stato_fisico == 2)
            {
                ...
                $magazzino_proprio = $obj_mag_generico->getMagazzinoUser("utilizzati","mag.label");
            ...
            // controlla ed effettua il tracking
            $etichetta_da_agggiornare = campo_db_da_aggiornare($db, $code_asset, "a.label", $etichetta);
            $stato_fisico_da_agggiornare = campo_db_da_aggiornare($db, $code_asset, "a.stato_fisico", $stato_fisico);
            $stato_tecnico_da_agggiornare = campo_db_da_aggiornare($db, $code_asset, "a.stato_tecnico", $stato_tecnico);
            $magazzino_da_agggiornare = campo_db_da_aggiornare($db, $code_asset, "a.id_magazzino", $id_magazzino);
            ...
            $array_tracking['codice_asset'] = $code_asset;



            if ($etichetta_da_agggiornare)
            {
                $campo_etichetta_vecchio = getValCampo($db, $code_asset, "label");
                $array_tracking['old'] = $campo_etichetta_vecchio;
                $array_tracking['new'] = Addslashes($etichetta);
                $array_tracking['azione'] = "modifica etichetta asset ";
                $array_tracking['descrizione'] = Addslashes("Etichetta dell'asset " . $code_asset . " aggiornato");
                $obj_track->nuovo_tracking($array_tracking);
            }
            if ($stato_fisico_da_agggiornare)
            {
                $campo_stato_fisico_vecchio = getValCampo($db, $code_asset, "stato_fisico");
                $array_tracking['old'] = $campo_stato_fisico_vecchio;
                $array_tracking['new'] = $stato_fisico;
                $array_tracking['azione'] = "modifica stato fisico";
                $array_tracking['descrizione'] = Addslashes("Stato fisico dell'asset " . $code_asset . " aggiornato");
                $obj_track->nuovo_tracking($array_tracking);
            }
            if ($stato_tecnico_da_agggiornare)
            {
                $campo_stato_tecnico_vecchio = getValCampo($db, $code_asset, "stato_tecnico");
                $array_tracking['old'] = $campo_stato_tecnico_vecchio;
                $array_tracking['new'] = $stato_tecnico;
                $array_tracking['azione'] = "modifica stato tecnico";
                $array_tracking['descrizione'] = Addslashes("Stato tecnico dell'asset " . $code_asset . " aggiornato");
                $obj_track->nuovo_tracking($array_tracking);
            }
            if ($magazzino_da_agggiornare)
            {
                $campo_magazzino_vecchio = getValCampo($db, $code_asset, "id_magazzino");
                $array_tracking['old'] = $campo_magazzino_vecchio;
                $array_tracking['new'] = $id_magazzino;
                $array_tracking['azione'] = "modifica del magazzino";
                $array_tracking['descrizione'] = Addslashes("Modifica magazzino dell'asset " . $code_asset);
                $obj_track->nuovo_tracking($array_tracking);
            }
        ...
        if ($result)
        {
            ...
            print '<td><input name="num_ticket_interno" value="' . $asset['num_ticket_interno'] . '" size="30" "></tr>';

            print '<tr><td valign="top">' . "Numero ticket cliente" . '</td>';
            print '<td><input name="num_ticket_cliente" value="' . $asset['num_ticket_cliente'] . '" size="30" "></tr>';
...
function campo_db_da_aggiornare($db, $codice_asset = "", $chiave_campo = "", $valore_campo = "")
{
    if (empty($codice_asset))
    {
        return false;
    }
    $dynamic_where = "WHERE a.cod_asset LIKE '" . $codice_asset . "' AND ";
    if ($chiave_campo == "a.stato_fisico")
    {
        $dynamic_where .= " $chiave_campo = " . $valore_campo;
    } else if ($chiave_campo == "a.stato_tecnico")
    {
        $dynamic_where .= " $chiave_campo = " . $valore_campo;
    } else if ($chiave_campo == "a.id_magazzino")
    {
        $dynamic_where .= " $chiave_campo = " . $valore_campo;
    } else
    {
        $dynamic_where .= $chiave_campo . " LIKE " . $valore_campo;
    }
...
function getValCampo($db, $codice_asset = "", $nome_campo)
{
    ...
    $sql = "SELECT a." . $nome_campo;
    $sql .= " FROM " . MAIN_DB_PREFIX . "asset as a ";
    $sql .= "WHERE a.cod_asset LIKE '" . $codice_asset . "'";
    $result = $db->query($sql);
    if ($result)
    {
        $obj = $db->fetch_object($result);
        if (is_null($obj)) // non è stato trovato
        {
            return false;
        }
        return $obj->$nome_campo;
    }
    return false;
}
