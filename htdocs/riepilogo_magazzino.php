    ...
    if ($resql)
    {
        ...
        if (!empty($sel_magazzino))
        {
            if ($sel_magazzino == "OK")
            {
                ...
                $array_asset = array();
                $array_asset = $obj_asset->getAssetFromMagazzino($id_magazzino); //ottengo tutti gli asset del magazzino selezionato
                if (!empty($array_asset))
                {
                    ...
                    foreach ($array_asset as $key => $record)
                    {
                        if ($key == "asset")
                        {
                            for ($i = 0; $i < count($array_asset[$key]); $i++)
                            {
                                $asset = $array_asset[$key][$i];
