...
class FactureFournisseur extends CommonInvoice
{
   ...
   /**
     * Ricevuta invoice
     */
    const TYPE_RICEVUTA = 5;
    /**
     *  * Ricevuta Professionista invoice
     */
    const TYPE_PROF = 6;
    /**
     *   *  * Prestazione Occasionale invoice
     */
    const TYPE_OCCASIONALE = 7;
    /**
	 *	Constructor
	 *
     ...
     *    @param			string	$type				clone if called from CreateFromClone
    ...
    function create($user)
    {
        ...
        if ($resql)
        {
                ...
                if ($resql_insert)
                {
                    ...
                    $this->updateline(
                        ...
                        $this->lines[$i]->product_type,
                    		0,
                    		false,
                    		$this->lines[$i]->array_options	// added diciannove
                    );
                }
            }
            // Update total price
            ...
            // Diciannove: dolibarr 3.8 alpha --> NO PULL REQUEST
            if ($result > 0)
            {
            	$action='create';
...
// Start Diciannove: NO PULL REQUEST --> 3.8 alpha
...
// End Diciannove
        ...
        if ($resql_rows)
        {
            ...
            if ($num_rows)
            {
                ...
                while ($i < $num_rows)
                {
                    ...
                    $this->lines[$i]					= new  FactureSupplierLigne($this->db);
                    $this->lines[$i]->fk_facture_fourn = $this->id;
                    ...
                    require_once(DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php');
                    $extrafields=new ExtraFields($this->db);
                    $extralabels=$extrafields->fetch_name_optionals_label($this->table_element_line,true);
                    $this->lines[$i]->fetch_optionals($this->lines[$i]->rowid,$extralabels);
                    // End Diciannove
        ...
        if ($resql)
        {
            ...
            if ($result > 0)
            {
                ...
                if(empty($array_options))
                {
                	...
                	$extralabelsline = $extrafieldsline->fetch_name_optionals_label($this->table_element_line);
                	$array_options 	 = $extrafieldsline->getOptionalsFromPost($extralabelsline, '');
                }
                
                $facturesupplierligne = new FactureSupplierLigne($this->db);
                $facturesupplierligne->rowid=$idligne;
                $facturesupplierligne->id	=	$idligne;
                $facturesupplierligne->fetch($facturesupplierligne->rowid);
                $facturesupplierligne->array_options = $array_options;
                
                $result=$facturesupplierligne->insertExtraFields();
                ...
                if (! $notrigger)
                {
                    ...
                    include_once DOL_DOCUMENT_ROOT . '/core/class/interfaces.class.php';
                    $interface=new Interfaces($this->db);
                    $result=$interface->run_triggers('LINEBILL_SUPPLIER_CREATE',$facturesupplierligne,$user,$langs,$conf);
                    if ($result < 0) 
                    { 
                    		$this->error = $facturesupplierligne->error; // not good but now ok
                        $error++;
                        $this->errors=$interface->errors;
                        $this->db->rollback();
                        return -1;
     ...
     * @return    	int           				<0 if KO, >0 if OK
     */
    function updateline($id, $desc, $pu, $vatrate, $txlocaltax1=0, $txlocaltax2=0, $qty=1, $idproduct=0, $price_base_type='HT', $info_bits=0, $type=0, $remise_percent=0, $notrigger=false,$array_options='')
    {
        ...
        if ($resql)
        {
            ...
            $facturesupplierligne = new FactureSupplierLigne($this->db);
            $facturesupplierligne->rowid=$id;
            $facturesupplierligne->id	=	$id;
            $facturesupplierligne->fetch($facturesupplierligne->rowid);
            $facturesupplierligne->array_options = $array_options;
            ...
            if (! $notrigger)
            {
                ...
                $result=$interface->run_triggers('LINEBILL_SUPPLIER_UPDATE',$facturesupplierligne,$user,$langs,$conf);
                if ($result < 0) 
                {
                	  $this->error = $facturesupplierligne->error; // not good but now ok
                    $error++;
                    $facturesupplierligne->errors=$interface->errors;
                    $this->db->rollback();
                    return -1;
                }
                // Fin appel triggers
    ...
    function deleteline($rowid, $notrigger=0)
    {
      ...
      $error=0;
    	...
    	$facturesupplierligne = new FactureSupplierLigne($this->db);
    	$facturesupplierligne->rowid=$rowid;
    	$facturesupplierligne->id	=	$rowid;
    	$facturesupplierligne->fetch($facturesupplierligne->rowid);
    	    	
    	if (! $error)
    	{      
                $sql_take='SELECT * FROM '.MAIN_DB_PREFIX.'facture_fourn_det';
                $sql_take.= ' WHERE rowid = '.$rowid;
                $resql_take = $this->db->query($sql_take);
                
                if($resql_take){
                
                    $objtake= $this->db->fetch_object($resql_take);
                     
                }
                 $sql_take1='SELECT * FROM '.MAIN_DB_PREFIX.'facture_fourn_extrafields ';
                $sql_take1.= ' WHERE fk_object = '.$objtake->fk_facture_fourn;
                $resql_take1 = $this->db->query($sql_take1);
                
                if($resql_take1){
                
                    $objtake1= $this->db->fetch_object($resql_take1);
                     
                }
                
                 $rit_tot = $objtake1->ritenuta_acconto_facturebusinessita - $objtake->ritenuta_acconto;
                $albo_tot = $objtake1->albo_tot - $objtake->albo;
                $spese_tot = $objtake1->spese_tot - $objtake->spese;
                $bollo_tot=$objtake1->bollo_tot - $objtake->bollo;
                $iva_tot=price2num($objtake1->iva_tot,2) - price2num(((($objtake->pu_ht+$objtake->albo)*$objtake->tva_tx)/100),2);
                $netto_tot = $objtake1->netto_da_pagare_facturebusinessita + $objtake->ritenuta_acconto - ($objtake->pu_ht+(($objtake->albo*$objtake->tva_tx)/100)+$objtake->spese+$objtake->bollo+$objtake->albo+$objtake->tva);
    	...
    	if ((! $error) && (empty($conf->global->MAIN_EXTRAFIELDS_DISABLED))) // For avoid conflicts if trigger used
    	{
    		$result=$facturesupplierligne->deleteExtraFields();		// remove extra fields
    		if ($result < 0)
    		{
    			$error++;
    			dol_syslog(get_class($this)."::delete error -4 ".$this->error, LOG_ERR);
    	...
    	if (! $error && ! $notrigger)
    	{
    		...
    		include_once(DOL_DOCUMENT_ROOT . "/core/class/interfaces.class.php");
    		$interface=new Interfaces($this->db);
    		$result=$interface->run_triggers('LINEBILL_SUPPLIER_DELETE',$facturesupplierligne,$user,$langs,$conf);
    		if ($result < 0) {
        ...
        $sql_up='UPDATE '.MAIN_DB_PREFIX.'facture_fourn_extrafields SET ';
                 $sql_up.= ' ritenuta_acconto_facturebusinessita = '.$rit_tot;
                $sql_up.= ', albo_tot = '.$albo_tot;
                $sql_up.= ', spese_tot = '.$spese_tot;
                $sql_up.= ', bollo_tot = '.$bollo_tot;
                $sql_up.= ', iva_tot = '.$iva_tot;
                $sql_up.= ', netto_da_pagare_facturebusinessita = '.$netto_tot;
                 $sql_up.= ' WHERE fk_object = '.$objtake->fk_facture_fourn;
                $resql_up = $this->db->query($sql_up);
                //echo '<script>alert("ciao")</script>';
                  $sql3 = 'UPDATE llx_facture_fourn SET total_ttc = '.$netto_tot;
    ...
    function getNomUrl($withpicto=0,$option='',$max=0)
    {
        ...
        {
            $lien = '<a href="'.DOL_URL_ROOT.'/fourn/facture/fiche.php?mainmenu=accountancy&facid='.$this->id.'">';
            $lienfin='</a>';
        }
     ...
     */
    function createFromClone($socid=0,$invertdetail=0)
    {
        ...
        // Clear fields
        $this->ref_supplier=$langs->trans("CopyOf").' '.$this->ref_supplier;
        $this->author             = $user->id;
        $this->user_valid         = '';
        ...
        {
            if (isset($this->lines[$i]->info_bits) && ($this->lines[$i]->info_bits & 0x02) == 0x02)	// We do not clone line of discounts
            {
                unset($this->lines[$i]);
            }
        }
        ...
        if (! $error)
        {
        	...
        	include_once DOL_DOCUMENT_ROOT . '/core/class/interfaces.class.php';
        	$interface=new Interfaces($this->db);
        	$result=$interface->run_triggers('BILL_SUPPLIER_CLONE',$this,$user,$langs,$conf);
        	if ($result < 0) {
        		$error++; $this->errors=$interface->errors;
        	}
...
/**
 *	\class      	FactureSupplierLigne
 *	\brief      	Classe permettant la gestion des lignes de supplier factures lines
 *								Gere des lignes de la table llx_facture_fourn_det
 */
class FactureSupplierLigne  extends CommonInvoiceLine
{
	var $db;
	var $error;
	...
	/**
	 *	Load supplier invoice line from database
	 *
	...
	{
		$sql = 'SELECT f.rowid, f.fk_facture_fourn, f.ref as ref_supplier, f.description, f.pu_ht, f.pu_ttc, f.qty, f.remise_percent, f.tva_tx, f.tva';
		$sql.= ', f.localtax1_tx, f.localtax2_tx, f.total_localtax1, f.total_localtax2 ';
		$sql.= ', f.total_ht, f.tva as total_tva, f.total_ttc, f.fk_product, f.product_type, f.info_bits, f.ritenuta_acconto_perc, f.albo_perc, f.spese, f.bollo';
		$sql.= ', p.rowid as product_id, p.ref as product_ref, p.label as label, p.description as product_desc';
		$sql.= ' FROM '.MAIN_DB_PREFIX.'facture_fourn_det as f';
		$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'product as p ON f.fk_product = p.rowid';
		...
		if ($result)
		{
			...
			$this->product_ref			= $obj->product_ref;       // Internal reference
			$this->ref							= $obj->product_ref;       // deprecated.
			$this->ref_supplier			= $obj->ref_supplier;      // Reference product supplier TODO Rename field ref to ref_supplier into table llx_facture_fourn_det and llx_commande_fournisseurdet and update fields it into updateline
			$this->libelle					= $obj->label;             // This field may contains label of product (when invoice create from order)
			$this->product_desc			= $obj->product_desc;      // Description du produit
			...
			$this->ritenuta_acconto_perc		= $obj->ritenuta_acconto_perc;
                        $this->albo_perc                    = $obj->albo_perc;
                        $this->spese                    = $obj->spese;
                        $this->bollo                    = $obj->bollo;
			// Retreive all extrafield for fourn invoice line
			// fetch optionals attributes and labels
			require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
			$extrafields=new ExtraFields($this->db);
			$extralabels=$extrafields->fetch_name_optionals_label($this->table_element,true);
			$this->fetch_optionals($this->rowid,$extralabels);
	...
	function insert($notrigger=0)
	{
		...
		$this->rowid=$this->db->last_insert_id(MAIN_DB_PREFIX.'facture_fourn_det');
	...
	function delete()
	{
		...
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."facture_fourn_det WHERE rowid = ".$this->rowid;
		dol_syslog(get_class($this)."::delete sql=".$sql, LOG_DEBUG);
