	...
	*/
	...
	if ($absolute_discount > 0)
	{
		...
		else
		{
			...
			print '<br>';
			$form->form_remise_dispo($_SERVER["PHP_SELF"].'?mainmenu=accountancy&id='.$object->id,0,'remise_id',$object->thirdparty->id,$absolute_discount,$filter,$resteapayer);
		}
	}
	if ($absolute_creditnote > 0)
	{
		...
		else
		{
			...
			$form->form_remise_dispo($_SERVER["PHP_SELF"].'?mainmenu=accountancy&id='.$object->id,0,'remise_id_for_payment',$object->thirdparty->id,$absolute_creditnote,$filter,$resteapayer);
		}
	}
	...
	print '</td>';
	if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && ! empty($object->brouillon) && $user->rights->facture->creer) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?mainmenu=accountancy&action=editinvoicedate&amp;id='.$object->id.'">'.img_edit($langs->trans('SetDate'),1).'</a></td>';
	print '</tr></table>';
	...
	if ($object->type != Facture::TYPE_CREDIT_NOTE)
	{
		...
		{
			$form->form_date($_SERVER['PHP_SELF'].'?mainmenu=accountancy&id='.$object->id,$object->date,'invoicedate');
		}
		else
		{
	...
	print '</td>';
	if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editpaymentterm' && ! empty($object->brouillon) && $user->rights->facture->creer) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?mainmenu=accountancy&action=editpaymentterm&amp;id='.$object->id.'">'.img_edit($langs->trans('SetDate'),1).'</a></td>';
	print '</tr></table>';
	print '</td><td colspan="3">';
	if ($object->type != Facture::TYPE_CREDIT_NOTE)
	{
		...
		{
			$form->form_date($_SERVER['PHP_SELF'].'?mainmenu=accountancy&id='.$object->id,$object->date_lim_reglement,'paymentterm');
		}
		else
		{
	...
	print '</td>';
	if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editconditions' && ! empty($object->brouillon) && $user->rights->facture->creer) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?mainmenu=accountancy&action=editconditions&amp;id='.$object->id.'">'.img_edit($langs->trans('SetConditions'),1).'</a></td>';
	print '</tr></table>';
	print '</td><td colspan="3">';
	if ($object->type != Facture::TYPE_CREDIT_NOTE)
	{
	...
	print '</td>';
	if ($action != 'editmode' && ! empty($object->brouillon) && $user->rights->facture->creer) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?mainmenu=accountancy&action=editmode&amp;id='.$object->id.'">'.img_edit($langs->trans('SetMode'),1).'</a></td>';
	print '</tr></table>';
	print '</td><td colspan="3">';
	if ($action == 'editmode')
	{
		$form->form_modes_reglement($_SERVER['PHP_SELF'].'?mainmenu=accountancy&id='.$object->id,$object->mode_reglement_id,'mode_reglement_id');
	}
	else
	{
		$form->form_modes_reglement($_SERVER['PHP_SELF'].'?mainmenu=accountancy&id='.$object->id,$object->mode_reglement_id,'none');
	}
	print '</td></tr>';
	...
	if ($object->statut > 0 && $object->paye == 0 && $num == 0)
	{
		...
		{
			print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?mainmenu=accountancy&id='.$object->id.'&amp;action=new">'.$langs->trans("MakeWithdrawRequest").'</a>';
		}
		else
		{
	...
	*/
	...
	if ($result_sql)
	{
		...
		while ($i < $num)
		{
			...
			print '<td>&nbsp;</td>';
			print '<td>';
			print '<a href="'.$_SERVER['PHP_SELF'].'?mainmenu=accountancy&id='.$object->id.'&amp;action=delete&amp;did='.$obj->rowid.'">';
			print img_delete();
			print '</a></td>';
			print "</tr>\n";
			$i++;
