...
class Facture extends CommonInvoice
{
	...
	function create($user,$notrigger=0,$forceduedate=0)
	{
		...
		$this->brouillon = 1;
                $this->natura=trim($this->natura);
                $this->rif_amm=trim($this->rif_amm);
		...
		if ($this->fac_rec > 0)
		{
			...
			$this->remise_percent    = $_facrec->remise_percent;
                        $this->natura    = $_facrec->natura;
                        $this->rif_amm    = $_facrec->rif_amm;
			// Clean parametres
		...
		$sql.= ",".($this->ref_int?"'".$this->db->escape($this->ref_int)."'":"null");
                $sql.= ",".($this->natura?"'".$this->db->escape($this->natura)."'":"null");
                $sql.= ",".($this->rif_amm?"'".$this->db->escape($this->rif_amm)."'":"null");
	...
	function createFromCurrent($user,$invertdetail=0)
	{
		...
		$facture->date              = $this->date;
                $facture->natura              = $this->natura;
                $facture->rif_amm              = $this->rif_amm;
	...
	function getNomUrl($withpicto=0,$option='',$max=0,$short=0,$moretitle='')
	{
		...
		if ($option == 'withdraw') $url = DOL_URL_ROOT.'/compta/facture/prelevement.php?mainmenu=accountancy&facid='.$this->id;
		else $url = DOL_URL_ROOT.'/compta/facture.php?mainmenu=accountancy&facid='.$this->id;
	...
	function fetch($rowid, $ref='', $ref_ext='', $ref_int='')
	{
		...
		$sql = 'SELECT f.rowid,f.facnumber,f.ref_client,f.ref_ext,f.natura,f.rif_amm,f.ref_int,f.type,f.fk_soc,f.amount,f.tva, f.localtax1, f.localtax2, f.total, f.total_ttc, f.revenuestamp';
		...
		if ($result)
		{
			if ($this->db->num_rows($result))
			{
				...
				$this->type					= $obj->type;
                                $this->natura					= $obj->natura;
                                $this->rif_amm					= $obj->rif_amm;
	...
	function update($user=0, $notrigger=0)
	{
		...
		if (isset($this->ref_client)) $this->ref_client=trim($this->ref_client);
                if (isset($this->natura)) $this->natura=trim($this->natura);
                if (isset($this->rif_amm)) $this->rif_amm=trim($this->rif_amm);
		...
		$sql.= " increment=".(isset($this->increment)?"'".$this->db->escape($this->increment)."'":"null").",";
                $sql.= " natura=".(isset($this->natura)?"'".$this->db->escape($this->natura)."'":"null").",";
                $sql.= " rif_amm=".(isset($this->rif_amm)?"'".$this->db->escape($this->rif_amm)."'":"null").",";
	 ...
	 *	Set natura
	 *
	 *	@param     	string	$ref_client		natura
	 *	@return		int						<0 if KO, >0 if OK
	 */
	function set_natura($natura)
	{
		$sql = 'UPDATE '.MAIN_DB_PREFIX.'facture';
		if (empty($natura))
			$sql .= ' SET natura = NULL';
		else
			$sql .= ' SET natura = \''.$this->db->escape($natura).'\'';
		$sql .= ' WHERE rowid = '.$this->id;
		if ($this->db->query($sql))
		{
			$this->natura = $natura;
	...
	function set_rif_amm($rif_amm)
	{
		$sql = 'UPDATE '.MAIN_DB_PREFIX.'facture';
		if (empty($rif_amm))
			$sql .= ' SET rif_amm = NULL';
		else
			$sql .= ' SET rif_amm = \''.$this->db->escape($rif_amm).'\'';
		$sql .= ' WHERE rowid = '.$this->id;
		if ($this->db->query($sql))
		{
			$this->rif_amm = $rif_amm;
			return 1;
		}
		else
		{
