...
if ($action == 'confirm_clone' && $confirm == 'yes' && $user->rights->facture->creer) {
    if (1 == 0 && empty($_REQUEST["clone_content"]) && empty($_REQUEST["clone_receivers"])) {
        ...
        if ($object->fetch($id) > 0) {
            ...
            if ($result > 0) {
                header("Location: " . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $result);
                exit();
            } else {
...
// Change status of invoice
    ...
    if ($object->statut == 2 || ($object->statut == 3 && $object->close_code != 'replaced')) {
        ...
        if ($result > 0) {
            header('Location: ' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $id);
            exit();
        } else {
            setEventMessage($object->error, 'errors');
        }
...
// Delete invoice
    ...
    if ($result > 0) {
        header('Location: ' . DOL_URL_ROOT . '/compta/facture/list.php?mainmenu=accountancy');
        exit();
    } else {
        setEventMessage($object->error, 'errors');
        $action = '';
...
// Validation
    ...
    if (empty($date)) {
        ...
        header('Location: ' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $id . '&action=editinvoicedate');
        exit;
    }
    $object->date = $date;
 ...
 */
    ...
    $object->fetch($id);
    $object->date_lim_reglement = dol_mktime(12, 0, 0, $_POST, $_POST, $_POST);
    if ($object->date_lim_reglement < $object->date)
        $object->date_lim_reglement = $object->date;
    $result = $object->update($user);
    if ($result < 0)
...
} else if ($action == 'set_natura' && $user->rights->facture->creer) {
    $object->fetch($id);
    $object->set_natura($_POST['natura']);
} else if ($action == 'set_rif_amm' && $user->rights->facture->creer) {
    $object->fetch($id);
    $object->set_rif_amm($_POST['rif_amm']);
}
 ...
 */ else if ($action == 'add' && $user->rights->facture->creer) {
    ...
    if ($_POST['type'] == Facture::TYPE_REPLACEMENT) {
        ...
        if (!$error) {
            ...
            $object->natura = $_POST['natura'];
            $object->rif_amm = $_POST['rif_amm'];
...
$object->rif_amm = $_POST['rif_amm'];
    ...
    if (($_POST['type'] == 0 || $_POST['type'] == 3 || $_POST['type'] == 4) && $_POST['fac_rec'] <= 0) {
        ...
        if (!$error) {
            ...
            $object->note_public = trim($_POST['note_public']);
            $object->note_private = trim($_POST['note_private']);
            //$object->num_ordine         = trim($_POST['options_num_ordine']);
            //$object->cig                = trim($_POST['options_cig']);
            //$object->oggetto_fatt       = trim($_POST['options_oggetto_fatt']);
            $object->ref_client = $_POST['ref_client'];
            $object->ref_int = $_POST['ref_int'];
            $object->modelpdf = $_POST['model'];
            $object->rif_amm = $_POST['rif_amm'];
                ...
                if ($id > 0) {
                    ...
                    if ($_POST['type'] == 3) {
                        ...
                        $result = $object->addline(
                                $langs->trans('Deposit'), $amountdeposit, // subprice
                                1, // quantity
...
// Add a new line
    ...
    header('Location: ' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $id); // Pour reaffichage de la fiche en cours d'edition
    exit();
}
...
// Modify line position (up)
    ...
    header('Location: ' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id . '#' . $_GET ['rowid']);
    exit();
            ...
            if ($mailfile->error) {
                ...
                if ($result) {
                    ...
                    if ($error) {
                        ...
                        header('Location: ' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id);
                        exit();
                    }
                } else {
                    $langs->load("other");
...
if ($action == 'create') {
    ...
    foreach ($facids as $key => $valarray) {
        ...
        //if(isset($_GET['socid'])&&isset($_GET['facid'])) $key=$_GET['facid'];
        
        $optionsav .= '<option value="' . $key . '"';
        if ($key == $_POST['fac_avoir']|$newinvoice_static->id == $_GET['facid'])
    ...
    if ($socid > 0) {
        ...
        print '<input type="radio" name="type" id="radio_replacement" value="4"' . (GETPOST('type') == 1 ? ' checked=true' : '');
        if (!$options)
    ...
    if (empty($origin)) {
        if ($socid > 0) {
            ...
            print '<input type="radio" id="radio_creditnote" name="type" value="2" checked="checked"' . (GETPOST('type') == 2 ? ' checked="checked"' : '');
            if (!$optionsav)
                print ' disabled="disabled"';
            print '>';
    ...
    $form->select_conditions_paiements(isset($_POST['cond_reglement_id']) ? $_POST['cond_reglement_id'] : $cond_reglement_id, 'cond_reglement_id', '', 1);
    print '</td></tr>';
    ...
    //Natura
    print '<tr><td colspan="1">' . $langs->trans('Natura') . '</td><td colspan="2"><input type="text" name="natura" id="natura" value="'.$object->natura.'"></td></tr>';
    print '<tr><td colspan="1">' . $langs->trans('Rif. Norm.') . '</td><td colspan="2"><input type="text" name="rif_amm" id="rif_amm" value="'.$object->rif_amm.'"></td></tr>';
    ...
    if ($action == 'valid') {
        ...
        $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id, $langs->trans('ValidateBill'), $text, 'confirm_valid', $formquestion, (($object->type != Facture::TYPE_CREDIT_NOTE && $object->total_ttc < 0) ? "no" : "yes"), ($conf->notification->enabled ? 0 : 2));
    }
    ...
    if ($action != 'refclient' && !empty($object->brouillon))
        print '<td align="right"><a href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&action=refclient&amp;id=' . $object->id . '">' . img_edit($langs->trans('Modify')) . '</a></td>';
    print '</tr></table>';
    print '</td>';
    print '<td colspan="5">';
    if ($user->rights->facture->creer && $action == 'refclient') {
        print '<form action="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&id=' . $object->id . '" method="post">';
        print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
    ...
    if (empty($conf->global->FACTURE_CHANGE_THIRDPARTY) && $action != 'editthirdparty' && $object->brouillon && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=editthirdparty&amp;facid=' . $object->id . '">' . img_edit($langs->trans('SetLinkToThirdParty'), 1) . '</a></td>';
    print '</tr></table>';
    print '</td><td colspan="5">';
    if ($action == 'editthirdparty') {
        $form->form_thirdparty($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->socid, 'socid');
    } else {
    ...
    print '<tr><td>';
    print '<table class="nobordernopadding" width="100%">';
    print '<tr><td>' . $langs->trans('Natura') . '</td>';
    print '</td><td colspan="5">';
    if ($action != 'editnatura' && $object->brouillon && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?action=editnatura&mainmenu=accountancy&amp;facid=' . $object->id . '">' . img_edit($langs->trans('Natura'), 1) . '</a></td>';
    print '</tr></table>';
     print '<td colspan="5">';
    if ($user->rights->facture->creer && $action == 'editnatura') {
        print '<form action="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&id=' . $object->id . '" method="post">';
        print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
        print '<input type="hidden" name="action" value="set_natura">';
        print '<input type="text" class="flat" size="20" name="natura" value="' . $object->natura . '">';
        print ' <input type="submit" class="button" value="' . $langs->trans('Modify') . '">';
        print '</form>';
    } else {
        print $object->natura;
    ...
    print '<tr><td>';
    print '<table class="nobordernopadding" width="100%">';
    print '<tr><td>' . $langs->trans('Rif. Norm.') . '</td>';
    print '</td><td colspan="5">';
    if ($action != 'edit_rif_amm' && $object->brouillon && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?action=edit_rif_amm&mainmenu=accountancy&amp;facid=' . $object->id . '">' . img_edit($langs->trans('Natura'), 1) . '</a></td>';
    print '</tr></table>';
     print '<td colspan="5">';
    if ($user->rights->facture->creer && $action == 'edit_rif_amm') {
        print '<form action="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&id=' . $object->id . '" method="post">';
        print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
        print '<input type="hidden" name="action" value="set_rif_amm">';
        print '<input type="text" class="flat" size="20" name="rif_amm" value="' . $object->rif_amm . '">';
        print ' <input type="submit" class="button" value="' . $langs->trans('Modify') . '">';
        print '</form>';
    } else {
        print $object->rif_amm;
    }
    print '</td></tr>';
    ...
    $addrelativediscount = '<a href="' . DOL_URL_ROOT . '/comm/remise.php?id=' . $soc->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?mainmenu=accountancy&facid=' . $object->id . '">' . $langs->trans("EditRelativeDiscounts") . '</a>';
    $addabsolutediscount = '<a href="' . DOL_URL_ROOT . '/comm/remx.php?id=' . $soc->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?mainmenu=accountancy&facid=' . $object->id . '">' . $langs->trans("EditGlobalDiscounts") . '</a>';
    $addcreditnote = '<a href="' . DOL_URL_ROOT . '/compta/facture.php?mainmenu=accountancy&action=create&socid=' . $soc->id . '&type=2&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?mainmenu=accountancy&facid=' . $object->id . '">' . $langs->trans("AddCreditNote") . '</a>';
    ...
    if ($absolute_discount > 0) {
        ...
        if ($object->statut > 0 || $object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT) {
            ...
            print '<br>';
            $form->form_remise_dispo($_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id, GETPOST('discountid'), 'remise_id', $soc->id, $absolute_discount, $filterabsolutediscount, $resteapayer, ' (' . $addabsolutediscount . ')');
        }
    } else {
    ...
    if ($absolute_creditnote > 0) {
        ...
        if ($object->statut != 1 || $object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT) {
            ...
            $form->form_remise_dispo($_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id, 0, 'remise_id_for_payment', $soc->id, $absolute_creditnote, $filtercreditnote, 0); // We
            // must
            // allow
            // credit
            // not
    ...
    if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && !empty($object->brouillon) && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=editinvoicedate&amp;facid=' . $object->id . '">' . img_edit($langs->trans('SetDate'), 1) . '</a></td>';
    print '</tr></table>';
    print '</td><td colspan="3">';

    if ($object->type != Facture::TYPE_CREDIT_NOTE) {
        if ($action == 'editinvoicedate') {
            $form->form_date($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->date, 'invoicedate');
        } else {
    ...
    if ($object->type != Facture::TYPE_CREDIT_NOTE) {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                print '<td align="right">';
                print '<a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id . '&action=unlinkdiscount&discountid=' . $obj->rowid . '">' . img_delete() . '</a>';
                print '</td></tr>';
                $i ++;
                if ($invoice->type == Facture::TYPE_CREDIT_NOTE)
    ...
    if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editconditions' && !empty($object->brouillon) && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=editconditions&amp;facid=' . $object->id . '">' . img_edit($langs->trans('SetConditions'), 1) . '</a></td>';
    print '</tr></table>';
    print '</td><td colspan="3">';
    if ($object->type != Facture::TYPE_CREDIT_NOTE) {
        if ($action == 'editconditions') {
            $form->form_conditions_reglement($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->cond_reglement_id, 'cond_reglement_id');
        } else {
            $form->form_conditions_reglement($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->cond_reglement_id, 'none');
        }
    } else {
        print '&nbsp;';
    }
    ...
    if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editpaymentterm' && !empty($object->brouillon) && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=editpaymentterm&amp;facid=' . $object->id . '">' . img_edit($langs->trans('SetDate'), 1) . '</a></td>';
    print '</tr></table>';
    print '</td><td colspan="3">';
    if ($object->type != Facture::TYPE_CREDIT_NOTE) {
        if ($action == 'editpaymentterm') {
            $form->form_date($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->date_lim_reglement, 'paymentterm');
        } else {
    ...
    if ($action != 'editmode' && !empty($object->brouillon) && $user->rights->facture->creer)
        print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=editmode&amp;facid=' . $object->id . '">' . img_edit($langs->trans('SetMode'), 1) . '</a></td>';
    print '</tr></table>';
    print '</td><td colspan="3">';
    if ($action == 'editmode') {
        $form->form_modes_reglement($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->mode_reglement_id, 'mode_reglement_id', 'CRDT');
    } else {
        $form->form_modes_reglement($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->mode_reglement_id, 'none', 'CRDT');
    }
    print '</td></tr>';
    ...
    if ($selleruserevenustamp) {  // Test company use revenue stamp
        ...
        if ($action != 'editrevenuestamp' && !empty($object->brouillon) && $user->rights->facture->creer)
            print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=editrevenuestamp&amp;facid=' . $object->id . '">' . img_edit($langs->trans('SetRevenuStamp'), 1) . '</a></td>';
        print '</tr></table>';
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print '</td>';
        if ($action != 'classify') {
            print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&action=classify&amp;facid=' . $object->id . '">';
        ...
        print '</td><td colspan="3">';
        if ($action == 'classify') {
            $form->form_project($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->socid, $object->fk_project, 'projectid');
        } else {
            $form->form_project($_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id, $object->socid, $object->fk_project, 'none');
        }
        print '</td>';
        print '</tr>';
    }
        ...
        foreach ($extrafields->attribute_label as $key => $label) {
            ...
            if ($extrafields->attribute_type [$key] == 'separate') {
                ...
                if ($action == 'edit_extras' && $user->rights->facture->creer && GETPOST('attribute') == $key) {
                    print '<form enctype="multipart/form-data" action="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy" method="post" name="formsoc">';
                    ...
                    if ($object->statut == 0 && $user->rights->facture->creer)
                        print '<a href="' . $_SERVER['PHP_SELF'] . '?id=' . $object->id . '&action=edit_extras&mainmenu=accountancy&attribute=' . $key . '">' . img_picto('', 'edit') . ' ' . $langs->trans('Modify') . '</a>';
                }
                print '</td></tr>' . "\n";
            }
        }
    ...
    print '	<form name="addproduct" id="addproduct" action="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&id=' . $object->id . (($action != 'editline') ? '#add' : '#line_' . GETPOST('lineid')) . '" method="POST">
        ...
        if (empty($reshook)) {
                ...
                if ($resteapayer == $object->total_ttc && $object->paye == 0 && $ventilExportCompta == 0) {
                    if (!$objectidnext) {
                        if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $user->rights->facture->valider) || $user->rights->facture->invoice_advance->unvalidate) {
                            print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=modif">' . $langs->trans('Modify') . '</a></div>';
                        } else {
            ...
            if ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT) || ($object->type == Facture::TYPE_CREDIT_NOTE && empty($discount->id)) || ($object->type == Facture::TYPE_DEPOSIT && empty($discount->id))) && ($object->statut == 2 || $object->statut == 3) && $user->rights->facture->creer) {    // A paid invoice (partially or completely)
                if (!$objectidnext && $object->close_code != 'replaced') {     // Not replaced by another invoice
                    print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=reopen">' . $langs->trans('ReOpen') . '</a></div>';
                } else {
            ...
            if ($object->statut == 0 && count($object->lines) > 0 && ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && (!empty($conf->global->FACTURE_ENABLE_NEGATIVE) || $object->total_ttc >= 0)) || ($object->type == Facture::TYPE_CREDIT_NOTE && $object->total_ttc <= 0))) {
                if ($user->rights->facture->valider) {
                    print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=valid">' . $langs->trans('Validate') . '</a></div>';
                }
            }
            ...
            if (($object->statut == 1 || $object->statut == 2) || !empty($conf->global->FACTURE_SENDBYEMAIL_FOR_ALL_STATUS)) {
                if ($objectidnext) {
                    ...
                    if (empty($conf->global->MAIN_USE_ADVANCED_PERMS) || $user->rights->facture->invoice_advance->send) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=presend&amp;mode=init">' . $langs->trans('SendByMail') . '</a></div>';
                    } else
            ...
            if (!empty($conf->global->FACTURE_SHOW_SEND_REMINDER)) {    // For backward compatibility
                if (($object->statut == 1 || $object->statut == 2) && $resteapayer > 0) {
                    if ($objectidnext) {
                        ...
                        if (empty($conf->global->MAIN_USE_ADVANCED_PERMS) || $user->rights->facture->invoice_advance->send) {
                            print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=prerelance&amp;mode=init">' . $langs->trans('SendRemindByMail') . '</a></div>';
                        } else
            ...
            if ($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT) {
                ...
                if ($object->type == Facture::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0 && $user->rights->facture->creer && $object->getSommePaiement() == 0) {
                    print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=converttoreduc">' . $langs->trans('ConvertToReduc') . '</a></div>';
                }
                // For deposit invoice
            ...
            if ($object->statut == 1 && $object->paye == 0 && $user->rights->facture->paiement && (($object->type != Facture::TYPE_CREDIT_NOTE && $object->type != Facture::TYPE_DEPOSIT && $resteapayer <= 0) || ($object->type == Facture::TYPE_CREDIT_NOTE && $resteapayer >= 0)) || ($object->type == Facture::TYPE_DEPOSIT && $object->paye == 0 && $resteapayer == 0 && $user->rights->facture->paiement && empty($discount->id))
                ...
                print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=paid">' . $langs->trans('ClassifyPaid') . '</a></div>';
            }
            ...
            if ($object->statut == 1 && $object->paye == 0 && $resteapayer > 0 && $user->rights->facture->paiement) {
                if ($totalpaye > 0 || $totalcreditnotes > 0) {
                    ...
                    print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=paid">' . $langs->trans('ClassifyPaidPartially') . '</a></div>';
                } else {
                    if ($objectidnext) {
                        ...
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=canceled">' . $langs->trans('ClassifyCanceled') . '</a></div>';
                    }
                }
            }
            ...
            if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $user->rights->facture->creer) {
                print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=clone&amp;object=invoice">' . $langs->trans("ToClone") . '</a></div>';
            }

            // Nota di credito
            if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $user->rights->facture->creer && $object->statut == 1) {
                print '<div class="inline-block divButAction"><a class="butAction" href="'.DOL_URL_ROOT.'/compta/facture.php?mainmenu=accountancy&action=create&socid='.$object->socid.'&facid='.$object->id.'">'.$langs->trans("AddBill").'</a></div>';
            }
            ...
            if ($user->rights->facture->supprimer) {
                if (!$object->is_erasable()) {
                    ...
                    print '<div class="inline-block divButAction"><a class="butActionDelete" href="' . $_SERVER["PHP_SELF"] . '?mainmenu=accountancy&facid=' . $object->id . '&amp;action=delete">' . $langs->trans('Delete') . '</a></div>';
                }
            } else {
    ...
    if ($action != 'prerelance' && $action != 'presend') {
        ...
        $urlsource = $_SERVER['PHP_SELF'] . '?mainmenu=accountancy&facid=' . $object->id;
