...
class Account extends CommonObject
{
    ...
    public $table_element_line='bank';		// Facture Business ITA
     ...
     */
    function addline($date, $oper, $label, $amount, $num_chq, $categorie, $user, $emetteur='',$banque='',$array_option=0)
    {
    	global $hookmanager;
        // Clean parameters
...
// Facture Business ITA            
            // Actions on extra fields (by external module or standard code)
            // FIXME le hook fait double emploi avec le trigger !!
            $hookmanager->initHooks(array('bankdao'));
            $bank_account_line_obj = new AccountLine($this->db);
            $bank_account_line_obj->id=$rowid;
            $bank_account_line_obj->fetch($bank_account_line_obj->id);
            
            if (is_array($array_option) && count($array_option)>0)
            {
            	$bank_account_line_obj->array_options=$array_option;
            }
            
            $parameters=array('id'=>$bank_account_line_obj->id);
            $reshook=$hookmanager->executeHooks('insertExtraFields',$parameters,$bank_account_line_obj,$action);    // Note that $action and $object may have been modified by some hooks
            
            if (empty($reshook))
            {
            	if (empty($conf->global->MAIN_EXTRAFIELDS_DISABLED)) // For avoid conflicts if trigger used
            	{
            		$result=$bank_account_line_obj->insertExtraFields();
            		if ($result < 0)
            		{
            			$error++;
            		}
    ...
    function fetch($id,$ref='')
    {
        ...
        if ($result)
        {
            if ($this->db->num_rows($result))
            {
                ...
                // Facture Business ITA
                // Retreive all extrafield for account
    ...
    function getNomUrl($withpicto=0, $mode='')
    {
        ...
        if (empty($mode))
        {
            $lien = '<a href="'.DOL_URL_ROOT.'/compta/bank/fiche.php?mainmenu=bank&id='.$this->id.'">';
            $lienfin='</a>';
        }
        else if ($mode == 'transactions')
        {
            $lien = '<a href="'.DOL_URL_ROOT.'/compta/bank/account.php?mainmenu=bank&account='.$this->id.'">';
            $lienfin='</a>';
        }
...
class AccountLine extends CommonObject
{
    ...
    function fetch($rowid,$ref='',$num='')
    {
        ...
        if ($result)
        {
            ...
            // Facture Business ITA
            // Retreive all extrafield for account line
            // fetch optionals attributes and labels
            require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
            $extrafields=new ExtraFields($this->db);
            $extralabels=$extrafields->fetch_name_optionals_label($this->table_element,true);
    ...
    function delete($user=0)
    {
        ...
        if (! $nbko)
        {
        	// Facture Business ITA
        	// Remove extrafields
        	if ((! $error) && ($conf->global->MAIN_MODULE_FACTUREBUSINESSITA)  && (empty($conf->global->MAIN_EXTRAFIELDS_DISABLED))) // For avoid conflicts if trigger used
        	{
        		$this->id=$this->rowid;
        		$result=$this->deleteExtraFields();
        		if ($result < 0)
