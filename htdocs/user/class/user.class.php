...
class User extends CommonObject
{
    ...
    var $lastname;
    var $firstname;
    var $tipologia; //scelta tipologia (aggiunto da Amin)
    var $note;
    var $email;
    var $skype;
    var $job;
    ...
    function fetch($id = '', $login = '', $sid = '', $loadpersonalconf = 1)
    {
        ...
        $sql.= " u.thm,";
        $sql.= " u.tipologia,"; // serve per caricare la tipologia (modificato da amin)
        ...
        if ($result)
        {
            ...
            if ($obj)
            {
                ...
                $this->accountancy_code = $obj->accountancy_code;
                 $this->tipologia =  $obj->tipologia; // tipologia (amin)
                $this->thm = $obj->thm;
        ...
        if ($resql)
        {
            ...
            } else
            {
                ...
                if ($result)
                {
                    ...
                    if ($this->tipologia != "N") // se nullo non creare il magazzino
                    {
                     ...
                     //   {
                            ...
                            $entrepot->libelle = $this->login; //etichetta (nome del magazzino) ora è il nome dell'utente creato
                            $entrepot->description = $langs->trans("ThisWarehouseIsPersonalStock", $this->getFullName($langs));
                            $entrepot->statut = 1;
                            $entrepot->country_id = $mysoc->country_id;
                            $id_magazzino_creato = $entrepot->create($user);
                            if ($id_magazzino_creato > 0)
                            {
                                //update tabella
                                $sql_update = "UPDATE " . MAIN_DB_PREFIX . "entrepot as e SET ";
                                $sql_update .= " e.fk_user = " . $this->id;
                                $sql_update .= " WHERE e.rowid = " .  $id_magazzino_creato ;
    ...
    function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0)
    {
        ...
        $this->tipologia = trim($this->tipologia); //aggiunto da amin
        ...
        $sql.= ", tipologia = '" . $this->db->escape($this->tipologia) . "'";
    ...
    function _load_ldap_info()
    {
        ...
        // Champs
        if ($this->fullname && !empty($conf->global->LDAP_FIELD_FULLNAME))
            $info[$conf->global->LDAP_FIELD_FULLNAME] = $this->fullname;
        if ($this->lastname && !empty($conf->global->LDAP_FIELD_NAME))
            $info[$conf->global->LDAP_FIELD_NAME] = $this->lastname;
        if ($this->firstname && !empty($conf->global->LDAP_FIELD_FIRSTNAME))
            $info[$conf->global->LDAP_FIELD_FIRSTNAME] = $this->firstname;
        if ($this->login && !empty($conf->global->LDAP_FIELD_LOGIN))
            $info[$conf->global->LDAP_FIELD_LOGIN] = $this->login;
        if ($this->login && !empty($conf->global->LDAP_FIELD_LOGIN_SAMBA))
            $info[$conf->global->LDAP_FIELD_LOGIN_SAMBA] = $this->login;
        if ($this->pass && !empty($conf->global->LDAP_FIELD_PASSWORD))
            $info[$conf->global->LDAP_FIELD_PASSWORD] = $this->pass; // this->pass = mot de passe non crypte
        if ($this->ldap_sid && !empty($conf->global->LDAP_FIELD_SID))
        ...
        if ($this->office_phone && !empty($conf->global->LDAP_FIELD_PHONE))
            $info[$conf->global->LDAP_FIELD_PHONE] = $this->office_phone;
        if ($this->user_mobile && !empty($conf->global->LDAP_FIELD_MOBILE))
            $info[$conf->global->LDAP_FIELD_MOBILE] = $this->user_mobile;
        if ($this->office_fax && !empty($conf->global->LDAP_FIELD_FAX))
            $info[$conf->global->LDAP_FIELD_FAX] = $this->office_fax;
        if ($this->note && !empty($conf->global->LDAP_FIELD_DESCRIPTION))
            $info[$conf->global->LDAP_FIELD_DESCRIPTION] = $this->note;
        if ($this->email && !empty($conf->global->LDAP_FIELD_MAIL))
            $info[$conf->global->LDAP_FIELD_MAIL] = $this->email;
        if ($this->skype && !empty($conf->global->LDAP_FIELD_SKYPE))
            $info[$conf->global->LDAP_FIELD_SKYPE] = $this->skype;
    ...
    function getNbOfUsers($limitTo = 'active', $all = false)
    {
        ...
        {
            if ($all)
                $sql.= " WHERE entity > 0"; // all users except superadmins
            else
